...
class Tank : public QGVDrawItem
{
    Q_OBJECT

public:
    explicit Tank();
    explicit Tank(const TargetItem& targetItem);
    explicit Tank(const QGV::GeoPos& geoPos, const double &heading = 0.0f,
                  const TargetItem::TargetState &state = TargetItem::Normal);
    ~Tank();

    // Set tank data from TargetItem
    void setTankData(const TargetItem& targetItem);
...
private:
    ...
    // Display properties
    QImage m_tankNormalImage;
    QImage m_tankDamageImage;
    QImage m_tankDestroyedImage;
    QImage m_tankUnknownImage;
    QSizeF m_tankIconSize;
    ...
    QPen getTankPen() const;
    QBrush getTankBrush() const;
    void loadImages();
