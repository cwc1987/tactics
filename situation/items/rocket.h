...
class Rocket : public QGVDrawItem
{
    Q_OBJECT

public:
    explicit Rocket();
    explicit Rocket(const TargetItem& targetItem);
    explicit Rocket(const QGV::GeoPos& geoPos, const double &heading = 0.0f,
                    const TargetItem::TargetState &state = TargetItem::Normal);
    ~Rocket();

    // Set rocket data from TargetItem
    void setRocketData(const TargetItem& targetItem);
...
private:
    ...
    // Display properties
    QImage m_rocketNormalImage;
    QImage m_rocketDamageImage;
    QImage m_rocketDestroyedImage;
    QImage m_rocketUnknownImage;
    QSizeF m_rocketIconSize;
    ...
    QPen getRocketPen() const;
    QBrush getRocketBrush() const;
    void loadImages();
