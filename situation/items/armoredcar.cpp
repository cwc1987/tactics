...
ArmoredCar::ArmoredCar(const TargetItem& targetItem)
    : m_altitude(0.0)
    , m_heading(0.0)
    , m_speed(0.0)
    , m_verticalSpeed(0.0)
    ...
    , m_isActive(true)
{
    setArmoredCarData(targetItem);
    loadImages();
    setFlag(QGV::ItemFlag::Transformed);
}

ArmoredCar::ArmoredCar(const QGV::GeoPos& geoPos, const double &heading, const TargetItem::TargetState &state)
    : m_geoPos(geoPos)
    , m_altitude(0.0)
    , m_heading(heading)
    , m_speed(0.0)
...
void ArmoredCar::setArmoredCarData(const TargetItem& targetItem)
{
    ...
    if(!qFuzzyCompare(m_geoPos.latitude(), targetItem.latitude) ||
        !qFuzzyCompare(m_geoPos.longitude(), targetItem.longitude)) {
        updatePosition = true;
    }
...
void ArmoredCar::onProjection(QGVMap *geoMap)
{
    if(geoMap == nullptr)
        return;
...
QPainterPath ArmoredCar::projShape() const
{
    updatePath();
    return m_cachedPath;
}

void ArmoredCar::projPaint(QPainter *painter)
{
    if (!painter) {
        return;
    }
    ...
    double wvalidSize = m_armoredCarIconSize.width() / state.scale();
    double hvalidSize = m_armoredCarIconSize.height() / state.scale();
    QRectF projRect(m_projPos - QPointF(wvalidSize/2, hvalidSize/2),
                    m_projPos + QPointF(wvalidSize/2, hvalidSize/2));

    // Draw armored car image based on state
    if(m_state == TargetItem::Normal)
        painter->drawImage(projRect, m_armoredCarNormalImage);
    else if(m_state == TargetItem::Damage)
        painter->drawImage(projRect, m_armoredCarDamageImage);
    else if(m_state == TargetItem::Destoried)
        painter->drawImage(projRect, m_armoredCarDestroyedImage);
    else
        painter->drawImage(projRect, m_armoredCarUnknownImage);
    
    // Draw armored car label if name is not empty and active
    if (!m_name.isEmpty() && m_isActive) {
        ...
        // Calculate text position (above the armored car)
...
QString ArmoredCar::projTooltip(const QPointF& projPos) const
{
    Q_UNUSED(projPos)
    
    QString tooltip = QString(tr("Armored Car: %1\nType: %2\nModel: %3\nState: %4\nThreat Level: %5\nConfidence: %6%\nAltitude: %7 m\nSpeed: %8 m/s\nMissiles: %9"))
...
QPointF ArmoredCar::projAnchor() const
{
    const QGVCameraState state = getMap()->getCamera();
    double wvalidSize = m_armoredCarIconSize.width() / state.scale();
    double hvalidSize = m_armoredCarIconSize.height() / state.scale();
...
QTransform ArmoredCar::projTransform() const
{
    return QGV::createTransfromAzimuth(projAnchor(), m_heading);
}

void ArmoredCar::updatePath() const
{
    m_cachedPath.clear();
...
QColor ArmoredCar::getDisplayColor() const
{
    switch (m_state) {
    case TargetItem::Normal:
        return QColor(0, 255, 0);      // Green - Normal
    case TargetItem::Damage:
        return QColor(255, 0, 0);      // Red - Damaged
    case TargetItem::Destoried:
        return QColor(128, 128, 128);  // Gray - Destroyed
    case TargetItem::UnkonwnState:
        return QColor(128, 128, 128);  // Gray - Unknown state
    default:
        return QColor(128, 128, 128);  // Default gray
    }
}

QString ArmoredCar::getStateString() const
{
    switch (m_state) {
    case TargetItem::Normal: 
        return tr("Normal");
...
QString ArmoredCar::getThreatLevelString() const
{
...
QString ArmoredCar::getTypeString() const
{
    switch (m_type) {
...
QPen ArmoredCar::getArmoredCarPen() const
{
...
QBrush ArmoredCar::getArmoredCarBrush() const
{
    QColor color = getDisplayColor();
    if (!m_isActive) {
        color.setAlpha(128);  // 50% opacity for inactive armored cars
    }
    return QBrush(color);
}

void ArmoredCar::loadImages()
{
    // Use tank images as placeholder for armored car
    // You can add specific armored car images later
    ...
    m_armoredCarNormalImage = QImage(normalPath);
    m_armoredCarDamageImage = QImage(damagePath);
    m_armoredCarDestroyedImage = QImage(destroyedPath);
    m_armoredCarUnknownImage = QImage(unknownPath);
    
    // Use tank size setting as default for armored car
    // You can add a specific ArmoredCarSize setting later
    QVariant armoredCarSize = Settings::instance()->getValue(SettingsKey::TankSize);
    if (armoredCarSize.isValid() && armoredCarSize.canConvert<QSizeF>()) {
        m_armoredCarIconSize = armoredCarSize.toSizeF();
    } else {
        // Fallback to airplane size
        m_armoredCarIconSize = Settings::instance()->getValue(SettingsKey::AirplaneSize).toSizeF();
    }
}

QColor ArmoredCar::getTextColorForBackground(const QColor &background) const
{
